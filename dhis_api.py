...
def get_pageSize(resource, auth, url):
    ...
    page_size = requests.get(
        url + f'{resource}', auth=auth).json().get('pager').get('total')
    return page_size
...
@retry(stop=stop_after_attempt(5))
def set_name_from_index(df, url, column_name,
                        index_table=None, fetch_index=False, auth=None):
    if fetch_index:
        assert auth is not None, "Cannot retrieve the index tables \
            without authentication object"
        index_table = get_dhis_index_table(
            auth, url, table=column_name + 's')
    else:
        assert index_table is not None, 'Set fetch index or \
