...
def pivot_stack(df):
    pivot_outliers = df.pivot_table(
        index=["orgUnit", "dataElement"], columns=["year", "month"]
    ).copy()

    pivot_outliers.rename(columns={"value": "with_outliers"}, level=0, inplace=True)
...
def replace_outliers(pivot_outliers, cutoff):
    ...
    pivot_no_outliers.rename(
        columns={"with_outliers": "without_outliers"}, level=0, inplace=True
    )
    ...
    for x in pivot_outliers.index:
        ...
        pivot_no_outliers.iloc[pivot_outliers.index.get_loc(x), :] = new_values.astype(
            "float"
        )
...
def replace_outliers_iqr(pivot_outliers, k):
    ...
    for x in pivot_outliers.index:
        ...
        if np.nanstd(values) != 0 and np.isnan(values).sum() != len(values):
            ...
            UB = Q3 + k * IQR
            new_values = np.where(
                (values < LB) | (values > UB), np.nanmedian(values), values
            )
        else:
            new_values = values
...
def add_info_and_format(
    ...
    data = pd.merge(
        data, location, left_on="orgUnit", right_on="facilitycode", how="left"
    )
...
def full_pivot_for_export(
    data, index=["district_name", "facility_id", "facility_name", "date"]
):

    data_pivot = data.pivot_table(index=index, columns=["dataElement"], aggfunc="sum")
...
def add_report_to_pivot(data_pivot, report, location):
    ...
    full_pivot = pd.merge(
        report_pivot, data_pivot, how="left", left_index=True, right_index=True
    )
...
def add_report_columns(data):
    ...
    """
    Transforms values in each indicator column into a flag of its reporting status.
    Takes in the value reported for an indicator, and the facilities expected and actual reporting status.
    Replaces the value reported by a flag, 1 or 0, and combines those three columns using odd numbers
        such that each combination of columns yields a unique result.
    As a result :
        -1 stands for unexpected reporting patterns
         0 stands for not expected to report
         1 stands for expected to report but did not report
         2 stands for reported a null or zero for this indicator
         3 stands for eported more than zero on this indicator
    """

    value_dict = {18: 3, 11: 2, 10: 1, 0: 0, 17: -1, 7: -1, 8: -1, 1: -1}

    data["e"] = data["expected_105_1_reporting"] * 9
    data["a+e"] = data[["expected_105_1_reporting", "actual_105_1_reporting"]].sum(
        axis=1
    )
