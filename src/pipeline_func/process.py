...
def add_report_columns(data):
    ...
    cols = list(VAR_CORR[VAR_CORR['domain'] != 'REPORT']
                ['identifier'].unique())
...
def process(data):
    ...
    report_indics = list(VAR_CORR[VAR_CORR['domain'] == 'REPORT']
                         ['identifier'].unique())
    data_df_noreport = data[~data['dataElement'].isin(
        report_indics)].copy()
    ...
    return (pivot_final, facility_data_reporting, facility_data_with_outliers, facility_data_no_outliers_std,
